...
public class SupplierPaymentController {
	...
	public String saveSupplierPayment(@RequestBody SupplierPayment supplierPayment) {
		...
		try {
			...
			List<Grn> incompleteGrns = grnController.findAllIncompleteBySupplier(supplierPayment.getSupplierId().getId());
			...
			for(Grn grn: incompleteGrns) {
				if (!paidAmount.equals(BigDecimal.ZERO)) {
					...
					if (comparison <= 0) {
						...
						GrnHasSupplierPayment grnHasSupplierPayment = new GrnHasSupplierPayment(grnBalanceAmount,grn,newSupplierPayment);
						...
						//save new grn has supplier payment record
						GrnHasSupplierPayment grnHasSupplierPayment = new GrnHasSupplierPayment(paidAmount,grn,newSupplierPayment);
						grnSupplierPaymentRepository.save(grnHasSupplierPayment);
