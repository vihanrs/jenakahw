...
public interface ReportRepository extends JpaRepository<PurchaseOrder, Integer> {
	...
	@Query(value = "SELECT MONTHNAME(DATE(invp.added_datetime)) as month_of_year, SUM(invp.paid_amount) as amount, 'Invoice' as type FROM jenakahw.invoice_has_payment invp "
			+ "WHERE MONTH(invp.added_datetime) = MONTH(CURDATE()) and YEAR(invp.added_datetime) = YEAR(CURDATE()) GROUP BY MONTHNAME(DATE(invp.added_datetime)) "
			+ "UNION ALL SELECT MONTHNAME(DATE(dein.added_datetime)) as month_of_year, SUM(dein.total) as amount, 'Extra Income' as type FROM daily_extra_income dein "
			+ "WHERE MONTH(dein.added_datetime) = MONTH(CURDATE()) and YEAR(dein.added_datetime) = YEAR(CURDATE()) AND dein.daily_income_expenses_status_id = 1 "
			+ " GROUP BY MONTHNAME(DATE(dein.added_datetime))  UNION ALL "
			+ "SELECT MONTHNAME(DATE(sp.added_datetime)) as month_of_year, SUM(sp.paid_amount) as amount, 'Supplier Payment' as type FROM supplier_payment sp "
			+ "WHERE MONTH(sp.added_datetime) = MONTH(CURDATE()) and YEAR(sp.added_datetime) = YEAR(CURDATE()) GROUP BY MONTHNAME(DATE(sp.added_datetime)) "
			+ "UNION ALL SELECT MONTHNAME(DATE(dexp.added_datetime)) as month_of_year, SUM(dexp.total) as amount, 'Expense' as type FROM daily_expenses dexp "
			+ "WHERE MONTH(dexp.added_datetime) = MONTH(CURDATE()) and YEAR(dexp.added_datetime) = YEAR(CURDATE()) AND dexp.daily_income_expenses_status_id = 1 GROUP BY MONTHNAME(DATE(dexp.added_datetime))", nativeQuery = true)
